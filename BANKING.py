...
def speak(audio):
    engine.say(audio)
    engine.runAndWait()
speak('WELCOME TO OUR BANK MANAGEMENT SYSTEM')
...
'''    
    ...
    if button == options[0]:
        speak('''This project has been created by RAJRISHI MITRA, SAYETA BISWAS  and  HIMANSHU YADAV of class 12th SCIENCE  under the guidance of Mr Tanmay Suhane
...
#------------------------------------------------------------------------------------------
#               GETTING PASSWORD FOR AUTHENTICATION AND AUTHORIZATION
#------------------------------------------------------------------------------------------
...
if button == options[0]:
    ...
    values(7070294877,'MEGHA KUMARI','2004-07-21','NALANDA',7070294877,'meghakumarinld1@gmail.com','c',100000)'''    
    ...
    values(625100203,'TANMAY SARKAR','2002-03-21','MALDA',8745691233,'tanmay026@gmail.com','c',90000)'''
...
def customer():
    ...
    for i in a:
        ...
        if i[0] == x[1] and i[1].lower() == x[0].lower():
...
def main():
    ...
    cprint('''\t\t\t\t\t\t\t\t\t\t\t\t\t\t CREATED BY :- RAJRISHI MITRA
            \t\t\t\t\t\t\t\t\t\t\t\t\t\t\tSAYETA BISWAS
            \t\t\t\t\t\t\t\t\t\t\t\t\t\t\tHIMANSHU YADAV''',
            'red', attrs=['blink'])
